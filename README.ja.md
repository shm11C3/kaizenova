...
QAフェーズ(必要なら `AGENTS.md` にプロジェクト検証ステージとして宣言してください)、マルチエージェント委譲の追跡、PRサイズ計測、状態ファイルのバージョン移行機構は含めていません。理由と、この設計が避けている過剰設計の失敗パターンは [docs/DESIGN.md](docs/DESIGN.md) にまとめています。
