...
kaizenova は小さなステートマシン、**Claude Code / Codex** 両対応のライフサイクルHook、正本となる4つのSkillをリポジトリに導入します。Hookはタスクサイクルが未完了のままターンを終えることをブロックし、Skillが「完了」の定義を与えます。
